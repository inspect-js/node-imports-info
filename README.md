...
Note that `imports` resolution also uses conditions (`node`, `require`, `import`, `default`, etc), which are not field-specific; their version support is tracked in [`node-exports-info`](https://www.npmjs.com/package/node-exports-info)’s `getConditionsForCategory`.
...
## Categories
 ...
 - `pattern-trailers-no-dir-slash`: support for trailing-slash folder mappings (ending in `/`) was removed for these versions (`17 - 24.13 || 25 - 25.3`)
